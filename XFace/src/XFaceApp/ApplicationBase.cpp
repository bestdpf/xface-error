...
bool XFaceApp::ApplicationBase::createScriptProcessors()
{
  return true;
}

boost::shared_ptr<IScriptProcessor> ApplicationBase::getScriptProcessor(const std::string& name) const
{
  boost::shared_ptr<IScriptProcessor> ret = boost::shared_ptr<IScriptProcessor>(new IScriptProcessor());
}
